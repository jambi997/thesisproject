...
func _physics_process(delta):
	...
	if cangrow == true:
		$Sprite.scale.x += 0.02
		$Sprite.scale.y += 0.02
		$damagerad/CollisionShape2D.scale.x += 0.02
		$damagerad/CollisionShape2D.scale.y += 0.02
		...
		$Sprite.scale.x = 1
		$Sprite.scale.y = 1
		$damagerad/CollisionShape2D.scale.x = 1
		$damagerad/CollisionShape2D.scale.y = 1
...
func choose_action():
   ...
   #velocity = (target.position - position).normalized() * rush_speed
		states.CHILL:
			velocity = Vector2.ZERO
